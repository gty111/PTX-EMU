...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  const int numk = 10;//atoi(argv[1]); // number of points (unit: 1K)
  const int check = 1;//atoi(argv[2]); // do check for small problem sizes
  const int iterations = 100;//atoi(argv[3]); // repeat kernel execution
